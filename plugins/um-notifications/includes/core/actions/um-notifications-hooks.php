...
/**
 * Remove a web notification from count after updating conversation
 *
 * @param $user_id
 * @param $conversation_id
 */
add_action( 'um_ajax_messaging_after_update','um_notification_messaging_remove', 10, 2 );
function um_notification_messaging_remove( $user_id, $conversation_id ) {
	global $wpdb;

	// @todo async timer in version 3
	...
	foreach ( $ids as $id ) {
		UM()->Notifications_API()->api()->delete_notification_id_from_meta( $id['id'] );
	}
